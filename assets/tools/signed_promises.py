...
# Reads from a Dinartether node - make sure its running
...
def get_rpc_connection():
    ...
    rpc_connection = AuthServiceProxy("http://%s:%s@127.0.0.1:112021"%(rpc_user, rpc_pass))
...
def get_signed_assets(assets):
    ...
    print(assets)
    for key,value in assets.iteritems():
        if key == 'DINARTETHER_WITH_METADATA':
            print("Key " + key)
            #print("Value " + value)
