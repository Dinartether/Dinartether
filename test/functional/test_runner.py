...
`test/functional/test_framework/test_framework.py:DinartetherTestFramework.main`.
...
def main():
    ...
    tmpdir = "%s/dinartether_test_runner_%s" % (args.tmpdirprefix, datetime.datetime.now().strftime("%Y%m%d_%H%M%S"))
    os.makedirs(tmpdir)
    ...
    # Check that the build was configured with wallet, utils, and dinartetherd
    enable_wallet = config["components"].getboolean("ENABLE_WALLET")
    enable_cli = config["components"].getboolean("ENABLE_UTILS")
    enable_dinartetherd = config["components"].getboolean("ENABLE_DINTD")
    if not (enable_wallet and enable_cli and enable_dinartetherd):
        print("No functional tests to run. Wallet, utils, and dinartetherd must all be enabled")
...
def run_tests(test_list, src_dir, build_dir, exeext, tmpdir, use_term_control, jobs=1, enable_coverage=False, args=None, combined_logs_len=0, failfast=False, last_loop=False):
    # Warn if dinartetherd is already running (unix only)
    if args is None:
        args = []
    try:
        if subprocess.check_output(["pidof", "dinartetherd"]) is not None:
            print("%sWARNING!%s There is already a dinartetherd process running on this system. Tests may fail unexpectedly due to resource contention!" % (BOLD[1], BOLD[0]))
    ...
    #Set env vars
    if "DINTD" not in os.environ:
        os.environ["DINTD"] = build_dir + '/src/dinartetherd' + exeext
        os.environ["DINTCLI"] = build_dir + '/src/dinartether-cli' + exeext
...
class RPCCoverage:
    ...
    commands per `dinartether-cli help` (`rpc_interface.txt`).
