...
sudo find ${DISTNAME}/bin -type f -executable -exec ../dinartether/contrib/devtools/split-debug.sh {} {} {}.dbg \;
sudo find ${DISTNAME}/lib -type f -exec ../dinartether/contrib/devtools/split-debug.sh {} {} {}.dbg \;
...
make dist
SOURCEDIST=`echo dinartether-*.tar.gz`
mkdir -p temp
cd temp
tar xf ../$SOURCEDIST
find dinartether-* | sort | tar --no-recursion --mode='u+rw,go+r-w,a+X' --owner=0 --group=0 -c -T - | gzip -9n > ../$SOURCEDIST
cd ~/dinartether
...
cp -f dinartether-*setup*.exe ~/release/unsigned/
mkdir -p ~/win64
...
cd ~/
sudo rm -rf win64
cp -rf dinartether/contrib/windeploy ~/sign/win64
cd ~/sign/win64/windeploy
mkdir unsigned
mv ~/dinartether/dinartether-*setup-unsigned.exe unsigned/
...
make clean
export PATH=$PATH_orig
#transfer ~/sign/dinartether-*-win*-unsigned.tar.gz to the signing machine
```
...
#transfer dinartether-*-win*-unsigned.tar.gz to ~/win64/  
...
#transfer dinartether-*-win*-setup.exe back to the Ubuntu18 build machine to the folder ~/release (to shasum with the rest of the releases)
```
...
cd ~/
sudo rm -rf win32
cp -rf dinartether/contrib/windeploy ~/sign/win32
...
#transfer dinartether-*-win*-unsigned.tar.gz to ~/win32/  
tar xf $DISTNAME-win*-unsigned.tar.gz
rm $DISTNAME-win*-unsigned.tar.gz
./detached-sig-create.sh -key /path/to/codesign.pvk
#Enter the passphrase for the key when prompted
...
~/dinartether/contrib/macdeploy/extract-osx-sdk.sh
...
git clone https://github.com/Dinartether/Dinartether
mkdir ~/dinartether/depends/SDKs
#transfer MacOSX10.11.sdk.tar.gz to the folder dinartether/depends/SDKs
cd dinartether/depends/SDKs && tar -xf MacOSX10.11.sdk.tar.gz 
...
cd ~/dinartether
make deploy
$PWD/depends/x86_64-apple-darwin14/native/bin/dmg dmg "Dinartether-Core.dmg" ~/release/unsigned/$DISTNAME-osx-unsigned.dmg
...
make clean
export PATH=$PATH_orig
# Transfer ~/sign/dinartether-*-osx*-unsigned.tar.gz to a MacOS signing machine
```
...
cd ~/desktop
mkdir OSX
#transfer dinartether-*-osx-unsigned.tar.gz to ~/desktop/OSX
cd OSX
