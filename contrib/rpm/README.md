...
The RPM spec file provided here is for Dinartether-Core 0.12.0 and builds on CentOS
...
reference the release tarball available from https://dinartether.org/bin/ and
`Source1` should reference the BerkeleyDB source.

Sources 10-99 are for source files that are maintained in the
[Dinartether git repository](https://github.com/Dinartether/Dinartether) but are not part of
...
Currently only `Source10` is used, for the example `dinartether.conf` file.
...
in the dinartether git repository. At present only an SVG version of the Dinartether
icon is used.

## Patches

In general, patches should be avoided. When a packager feels a patch is
necessary, the packager should bring the problem to the attention of the dinartether
...
needed when 0.12.1 is released, a proper fix is already in the Dinartether git
master branch.
...
idea to build Dinartether Core against a static version of that library compiled
...
The `dinartether.svg` file is from the source listed as `Source100`. It is used as
...
matters for the `dinartether-server` package. At this point, most RPM based
...
The `dinartether-server` package should have SELinux support. How to properly do
...
Dinartether-Core against OpenSSL or use a patched version of Boost in the build
system.
...
[Ringing Liberty](https://www.ringingliberty.com/dinartether/). He has been
packaging Dinartether for Fedora at least since 2012.
...
4. I renamed the `dinartether` package that contains the Qt GUI to `dinartether-core` as
that appears to be how the general population refers to it, in contrast to
`dinartether-xt` or `dinartether-classic`. I wanted to make sure the general population
