...
The Dinartether Doxygen documentation can be generated by running `make docs` from the top-level 
...
Run with the -testnet option to run with "play dinartethers" on the test network, if you
...
Dinartether Core is a multithreaded application, and deadlocks or other multithreading bugs
...
- ThreadSocketHandler : Sends/Receives data from peers on port 12022.
...
- ThreadRPCServer : Remote procedure call handler, listens on port 12021 for connections and services them.

- DinartetherMiner : Generates dinartethers (if wallet is enabled).
...
However, in open source software such as Dinartether Core, where everyone uses
...
pay attention to for reviewers of Dinartether Core code.

General Dinartether Core
----------------------
...
- For `strprintf`, `LogPrint`, `LogPrintf` formatting characters don't need size specifiers
  ...
  - *Rationale*: Dinartether Core uses tinyformat, which is type safe. Leave them out to avoid confusion
...
be sent upstream but bugfixes may also be prudent to PR against Dinartether so that they can be integrated
...
- When looking at other's pull requests, it may make sense to add the following section to your `.git/config`
  file:
        ...
        [remote "upstream-pull"]
                ...
                url = git@github.com:dinartether/dinartether.git
...
- Try not to overload methods on argument type. E.g. don't make `getblock(true)` and `getblock("hash")`
  ...
  - *Rationale*: This is impossible to use with `dinartether-cli`, and can be surprising to users.
...
- Add every non-string RPC argument `(method, idx, name)` to the table `vRPCConvertParams` in `rpc/client.cpp`.
  ...
  - *Rationale*: `dinartether-cli` and the GUI debug console use this table to determine how to
