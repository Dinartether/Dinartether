...
The Dinartether Core daemon can be configured to act as a trusted "border
router", implementing the dinartether wire protocol and relay, making
...
The ZeroMQ feature in Dinartether Core requires the ZeroMQ API >= 4.0.0
...
during the *configure* step of building dinartetherd:
...
For instance:
    ...
    $ dinartetherd -zmqpubhashtx=tcp://127.0.0.1:212021 \
               -zmqpubrawtx=ipc:///tmp/dinartetherd.tx.raw
...
These options can also be provided in dinartether.conf.
...
From the perspective of dinartetherd, the ZeroMQ socket is write-only; PUB
sockets don't even have a read function. Thus, there is no state
introduced into dinartetherd directly. Furthermore, no information is
...
using. Dinartetherd appends an up-counting sequence number to each
