 ...
 * between dinartetherd, and dinartether-qt and non-server tools.
*/
...
void PrepareShutdown()
{
    ...
    RenameThread("dinartether-shutoff");
    ...
    FlushWallets();
#endif
    GenerateDinartethers(false, 0, GetParams());
...
std::string HelpMessage(HelpMessageMode mode)
{
    ...
    strUsage += HelpMessageOpt("-conf=<file>", strprintf(_("Specify configuration file (default: %s)"), DINARTETHER_CONF_FILENAME));
    if (mode == HMM_DINTD)
    {
#if HAVE_DECL_DAEMON
...
#ifndef WIN32
    strUsage += HelpMessageOpt("-pid=<file>", strprintf(_("Specify pid file (default: %s)"), DINARTETHER_PID_FILENAME));
#endif
...
{
    const std::string URL_SOURCE_CODE = "<https://github.com/Dinartether/Dinartether>";
    const std::string URL_WEBSITE = "<https://dinartether.com>";
...
void ThreadImport(std::vector<fs::path> vImportFiles)
{
    ...
    RenameThread("dinartether-loadblk");
...
/** Sanity checks
 *  Ensure that Dinartether is running in a usable environment with all
...
void InitLogging()
{
    ...
    LogPrintf("\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n");
    LogPrintf("Dinartether version %s\n", FormatFullVersion());
}
...
static bool LockDataDirectory(bool probeOnly)
{
    ...
    // Make sure only a single Dinartether process is using the data directory.
...
bool AppInitMain(boost::thread_group& threadGroup, CScheduler& scheduler)
{
    ...
    LogPrintf("Using config file %s\n", GetConfigFile(gArgs.GetArg("-conf", DINARTETHER_CONF_FILENAME)).string());
    ...
    GenerateDinartethers(fGenerate, gArgs.GetArg("-genproclimit", DEFAULT_GENERATE_THREADS), chainparams);
