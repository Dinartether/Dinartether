...
bool GenerateDistributionList(const CRewardSnapshot& p_rewardSnapshot, std::vector<OwnerAndAmount>& vecDistributionList)
{
    ...
    CAmount srcUnitDivisor = COIN;  //  Default to divisor for DINT
    ...
    if (p_rewardSnapshot.strDistributionAsset != "DINT") {
        ...
        LogPrint(BCLog::REWARDS, "%s: Distribution is DINT with divisor %d\n", __func__, srcUnitDivisor);
    }
        ...
        std::string& change_address, uint256& retTxid)
{
    ...
    //  Handle payouts using DINT differently from those using an asset
    if (p_rewardSnapshot.strDistributionAsset == "DINT") {
        // Check amount
        ...
        for (int i = start; i < (int)p_pendingPayments.size() && i < stop; i++) {
            ...
            // Parse Dinartether address (already validated during ownership snapshot creation)
