...
#include "dinartetheraddressvalidator.h"
...
void setupAddressWidget(QValidatedLineEdit *widget, QWidget *parent)
{
    ...
    widget->setPlaceholderText(QObject::tr("Enter a Dinartether address (e.g. %1)").arg(
        QString::fromStdString(DummyAddress(GetParams()))));
#endif
    widget->setValidator(new DinartetherAddressEntryValidator(parent));
    widget->setCheckValidator(new DinartetherAddressCheckValidator(parent));
}
...
bool parseDinartetherURI(const QUrl &uri, SendCoinsRecipient *out)
{
    // return if URI is not valid or is no dinartether: URI
    if(!uri.isValid() || uri.scheme() != QString("dinartether"))
        return false;
    ...
    for (QList<QPair<QString, QString> >::iterator i = items.begin(); i != items.end(); i++)
    {
        ...
        else if (i->first == "amount")
        {
            if(!i->second.isEmpty())
            {
                if(!DinartetherUnits::parse(DinartetherUnits::DINT, i->second, &rv.amount))
                {
                    return false;
                }
            }
...
bool parseDinartetherURI(QString uri, SendCoinsRecipient *out)
{
    // Convert dinartether:// to dinartether:
    //
    //    Cannot handle this later, because dinartether:// will cause Qt to see the part after // as host,
    //    which will lower-case it (and thus invalidate the address).
    if(uri.startsWith("dinartether://", Qt::CaseInsensitive))
    {
        uri.replace(0, 10, "dinartether:");
    }
    QUrl uriInstance(uri);
    return parseDinartetherURI(uriInstance, out);
}

QString formatDinartetherURI(const SendCoinsRecipient &info)
{
    QString ret = QString("dinartether:%1").arg(info.address);
    int paramCount = 0;

    if (info.amount)
    {
        ret += QString("?amount=%1").arg(DinartetherUnits::format(DinartetherUnits::DINT, info.amount, false, DinartetherUnits::separatorNever));
        paramCount++;
    }
...
bool openDinartetherConf()
{
    boost::filesystem::path pathConfig = GetConfigFile(DINARTETHER_CONF_FILENAME);
    ...
    /* Open dinartether.conf with the associated application */
...
fs::path static StartupShortcutPath()
{
    ...
    if (chain == CBaseChainParams::MAIN)
        return GetSpecialFolderPath(CSIDL_STARTUP) / "Dinartether.lnk";
    if (chain == CBaseChainParams::TESTNET) // Remove this special case when CBaseChainParams::TESTNET = "testnet4"
        return GetSpecialFolderPath(CSIDL_STARTUP) / "Dinartether (testnet).lnk";
    return GetSpecialFolderPath(CSIDL_STARTUP) / strprintf("Dinartether (%s).lnk", chain);
}

bool GetStartOnSystemStartup()
{
    // check for Dinartether*.lnk
...
fs::path static GetAutostartFilePath()
{
    std::string chain = ChainNameFromCommandLine();
    if (chain == CBaseChainParams::MAIN)
        return GetAutostartDir() / "dinartether.desktop";
    return GetAutostartDir() / strprintf("dinartether-%s.lnk", chain);
}
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    else
    {
        ...
        if (!optionFile.good())
            return false;
        std::string chain = ChainNameFromCommandLine();
        // Write a dinartether.desktop file to the autostart directory:
        optionFile << "[Desktop Entry]\n";
        optionFile << "Type=Application\n";
        if (chain == CBaseChainParams::MAIN)
            optionFile << "Name=Dinartether\n";
        else
            optionFile << strprintf("Name=Dinartether (%s)\n", chain);
...
LSSharedFileListItemRef findStartupItemInList(LSSharedFileListRef list, CFURLRef findUrl)
{
    ...
    // loop through the list of startup items and try to find the dinartether app
...
bool GetStartOnSystemStartup()
{
    CFURLRef dinartetherAppUrl = CFBundleCopyBundleURL(CFBundleGetMainBundle());
    if (dinartetherAppUrl == nullptr) {
        return false;
    }
    
    LSSharedFileListRef loginItems = LSSharedFileListCreate(nullptr, kLSSharedFileListSessionLoginItems, nullptr);
    LSSharedFileListItemRef foundItem = findStartupItemInList(loginItems, dinartetherAppUrl);

    CFRelease(dinartetherAppUrl);
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    if(fAutoStart && !foundItem) {
        // add dinartether app to startup item list
        LSSharedFileListInsertItemURL(loginItems, kLSSharedFileListItemBeforeFirst, nullptr, nullptr, dinartetherAppUrl, nullptr, nullptr);
    }
