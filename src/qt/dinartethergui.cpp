...
#include "dinartethergui.h"

#include "dinartetherunits.h"
...
const std::string DinartetherGUI::DEFAULT_UIPLATFORM =
#if defined(Q_OS_MAC)
        "macosx"
...
const QString DinartetherGUI::DEFAULT_WALLET = "~Default";
...
std::array<CurrencyUnitDetails, 5> CurrencyUnits::CurrencyOptions = { {
    { "BTC",    "DINTBTC"  , 1,          8},
    { "mBTC",   "DINTBTC"  , 1000,       5},
    { "µBTC",   "DINTBTC"  , 1000000,    2},
    { "Satoshi","DINTBTC"  , 100000000,  0},
    { "USDT",   "DINTUSDT" , 1,          5}
} };

static bool ThreadSafeMessageBox(DinartetherGUI *gui, const std::string& message, const std::string& caption, unsigned int style);

DinartetherGUI::DinartetherGUI(const PlatformStyle *_platformStyle, const NetworkStyle *networkStyle, QWidget *parent) :
    QMainWindow(parent),
    enableWallet(false),
...
void DinartetherGUI::createActions()
{
    QFont font = QFont();
    ...
    sendCoinsAction->setStatusTip(tr("Send coins to a Dinartether address"));
    ...
    receiveCoinsAction->setStatusTip(tr("Request payments (generates QR codes and dinartether: URIs)"));
    ...
    transferAssetAction->setStatusTip(tr("Transfer assets to DINT addresses"));
...
//    messagingAction->setShortcut(QKeySequence(Qt::ALT + Qt::Key_9));
    ...
    signMessageAction->setStatusTip(tr("Sign messages with your Dinartether addresses to prove you own them"));
    verifyMessageAction = new QAction(platformStyle->TextColorIcon(":/icons/verify"), tr("&Verify message..."), this);
    verifyMessageAction->setStatusTip(tr("Verify messages to ensure they were signed with specified Dinartether addresses"));
    ...
    openAction->setStatusTip(tr("Open a dinartether: URI or payment request"));

    showHelpMessageAction = new QAction(platformStyle->TextColorIcon(":/icons/info"), tr("&Command-line options"), this);
    showHelpMessageAction->setMenuRole(QAction::NoRole);
    showHelpMessageAction->setStatusTip(tr("Show the %1 help message to get a list with possible Dinartether command-line options").arg(tr(PACKAGE_NAME)));
...
void DinartetherGUI::createMenuBar()
{
#ifdef Q_OS_MAC
...
void DinartetherGUI::createToolBars()
{
    if(walletFrame)
    {
        QSettings settings;
        ...
        if(IconsOnly) {
            labelToolbar->setPixmap(QPixmap::fromImage(QImage(":/icons/dinttext")));
        }
        else {
            labelToolbar->setPixmap(QPixmap::fromImage(QImage(":/icons/dinartethertext")));
        }
...
//        m_toolbar->addAction(votingAction);
        ...
        QVBoxLayout* dinartetherLabelLayout = new QVBoxLayout(toolbarWidget);
        dinartetherLabelLayout->addWidget(labelToolbar);
        dinartetherLabelLayout->addWidget(m_toolbar);
        dinartetherLabelLayout->setDirection(QBoxLayout::TopToBottom);
        dinartetherLabelLayout->addStretch(1);
        ...
        labelCurrentMarket->setText(tr("Dinartether Market Price"));
        ...
        labelVersionUpdate->setText("<a href=\"https://github.com/Dinartether/Dinartether/releases\">New Wallet Version Available</a>");
        ...
        // Get the latest Dinartether release and let the user know if they are using the latest version
        // Network request code for the header widget
                       ...
                       if (key == "tag_name") {
                           ...
                           if (fNewSoftwareFound) {
                               ...
                               if (GetRandInt(2) == 1) {
                                   ...
                                   if (fRet) {
                                       QString link = "https://github.com/Dinartether/Dinartether/releases";
...
void DinartetherGUI::updateIconsOnlyToolbar(bool IconsOnly)
{
    if(IconsOnly) {
        labelToolbar->setPixmap(QPixmap::fromImage(QImage(":/icons/dinttext")));
        m_toolbar->setMaximumWidth(65);
...
void DinartetherGUI::setClientModel(ClientModel *_clientModel)
{
...
#ifdef ENABLE_WALLET
bool DinartetherGUI::addWallet(const QString& name, WalletModel *walletModel)
{
    if(!walletFrame)
        return false;
...
bool DinartetherGUI::setCurrentWallet(const QString& name)
{
    if(!walletFrame)
        return false;
...
void DinartetherGUI::removeAllWallets()
{
    if(!walletFrame)
        return;
    setWalletActionsEnabled(false);
...
void DinartetherGUI::setWalletActionsEnabled(bool enabled)
{
...
void DinartetherGUI::createTrayIcon(const NetworkStyle *networkStyle)
{
#ifndef Q_OS_MAC
...
void DinartetherGUI::createTrayIconMenu()
{
#ifndef Q_OS_MAC
...
#ifndef Q_OS_MAC
void DinartetherGUI::trayIconActivated(QSystemTrayIcon::ActivationReason reason)
{
...
void DinartetherGUI::optionsClicked()
{
...
void DinartetherGUI::aboutClicked()
{
    if(!clientModel)
        return;
...
void DinartetherGUI::showDebugWindow()
{
...
void DinartetherGUI::showDebugWindowActivateConsole()
{
    rpcConsole->setTabFocus(RPCConsole::TAB_CONSOLE);
    showDebugWindow();
}

void DinartetherGUI::showWalletRepair()
{
    rpcConsole->setTabFocus(RPCConsole::TAB_REPAIR);
    showDebugWindow();
}

void DinartetherGUI::showHelpMessageClicked()
{
    helpMessageDialog->show();
}

#ifdef ENABLE_WALLET
void DinartetherGUI::openClicked()
{
...
void DinartetherGUI::gotoOverviewPage()
{
    overviewAction->setChecked(true);
    if (walletFrame) walletFrame->gotoOverviewPage();
}

void DinartetherGUI::gotoHistoryPage()
{
    historyAction->setChecked(true);
    if (walletFrame) walletFrame->gotoHistoryPage();
}

void DinartetherGUI::gotoReceiveCoinsPage()
{
    receiveCoinsAction->setChecked(true);
    if (walletFrame) walletFrame->gotoReceiveCoinsPage();
}

void DinartetherGUI::gotoSendCoinsPage(QString addr)
{
    sendCoinsAction->setChecked(true);
    if (walletFrame) walletFrame->gotoSendCoinsPage(addr);
}

void DinartetherGUI::gotoSignMessageTab(QString addr)
{
    if (walletFrame) walletFrame->gotoSignMessageTab(addr);
}

void DinartetherGUI::gotoVerifyMessageTab(QString addr)
{
    if (walletFrame) walletFrame->gotoVerifyMessageTab(addr);
}

/** DINT START */
void DinartetherGUI::gotoAssetsPage()
{
    transferAssetAction->setChecked(true);
    if (walletFrame) walletFrame->gotoAssetsPage();
};

void DinartetherGUI::gotoCreateAssetsPage()
{
    createAssetAction->setChecked(true);
    if (walletFrame) walletFrame->gotoCreateAssetsPage();
};

void DinartetherGUI::gotoManageAssetsPage()
{
    manageAssetAction->setChecked(true);
    if (walletFrame) walletFrame->gotoManageAssetsPage();
};

void DinartetherGUI::gotoRestrictedAssetsPage()
{
...
void DinartetherGUI::updateNetworkState()
{
    ...
    if (clientModel->getNetworkActive()) {
        tooltip = tr("%n active connection(s) to Dinartether network", "", count) + QString(".<br>") + tr("Click to disable network activity.");
    } else {
...
void DinartetherGUI::setNumConnections(int count)
{
    updateNetworkState();
}

void DinartetherGUI::setNetworkActive(bool networkActive)
{
    updateNetworkState();
}

void DinartetherGUI::updateHeadersSyncProgressLabel()
{
...
void DinartetherGUI::setNumBlocks(int count, const QDateTime& blockDate, double nVerificationProgress, bool header)
{
    if (modalOverlay)
    {
        if (header)
...
void DinartetherGUI::message(const QString &title, const QString &message, unsigned int style, bool *ret)
{
    QString strTitle = tr("Dinartether"); // default title
...
void DinartetherGUI::changeEvent(QEvent *e)
{
...
void DinartetherGUI::closeEvent(QCloseEvent *event)
{
#ifndef Q_OS_MAC // Ignored on Mac
    if(clientModel && clientModel->getOptionsModel())
    {
...
void DinartetherGUI::showEvent(QShowEvent *event)
{
...
#ifdef ENABLE_WALLET
void DinartetherGUI::incomingTransaction(const QString& date, int unit, const CAmount& amount, const QString& type, const QString& address, const QString& label, const QString& assetName)
{
    // On new transaction, make an info balloon
    QString msg = tr("Date: %1\n").arg(date);
    if (assetName == "DINT")
        msg += tr("Amount: %1\n").arg(DinartetherUnits::formatWithUnit(unit, amount, true));
    else
        msg += tr("Amount: %1\n").arg(DinartetherUnits::formatWithCustomName(assetName, amount, MAX_ASSET_UNITS, true));
...
void DinartetherGUI::dragEnterEvent(QDragEnterEvent *event)
{
    // Accept only URIs
...
void DinartetherGUI::dropEvent(QDropEvent *event)
{
    if(event->mimeData()->hasUrls())
    {
...
bool DinartetherGUI::eventFilter(QObject *object, QEvent *event)
{
...
#ifdef ENABLE_WALLET
bool DinartetherGUI::handlePaymentRequest(const SendCoinsRecipient& recipient)
{
    // URI has to be valid
...
void DinartetherGUI::setHDStatus(int hdEnabled)
{
    QString icon = "";
...
void DinartetherGUI::setEncryptionStatus(int status)
{
    switch(status)
    {
...
void DinartetherGUI::showNormalIfMinimized(bool fToggleHidden)
{
    if(!clientModel)
        return;
...
void DinartetherGUI::toggleHidden()
{
    showNormalIfMinimized(true);
}

void DinartetherGUI::detectShutdown()
{
...
void DinartetherGUI::showProgress(const QString &title, int nProgress)
{
    if (nProgress == 0)
    {
...
void DinartetherGUI::setTrayIconVisible(bool fHideTrayIcon)
{
    if (trayIcon)
    {
...
void DinartetherGUI::showModalOverlay()
{
...
static bool ThreadSafeMnemonic(DinartetherGUI *gui, unsigned int style)
{
    bool modal = (style & CClientUIInterface::MODAL);
    // The SECURE flag has no effect in the Qt GUI.
    // bool secure = (style & CClientUIInterface::SECURE);
...
void DinartetherGUI::subscribeToCoreSignals()
{
...
void DinartetherGUI::unsubscribeFromCoreSignals()
{
...
void DinartetherGUI::toggleNetworkActive()
{
    if (clientModel) {
...
void DinartetherGUI::handleRestart(QStringList args)
{
    if (!ShutdownRequested())
    ...
    menu(0)
{
    ...
    QList<DinartetherUnits::Unit> units = DinartetherUnits::availableUnits();
    int max_width = 0;
    const QFontMetrics fm(font());
    for (const DinartetherUnits::Unit unit : units)
    {
    #ifndef QTversionPreFiveEleven
        max_width = qMax(max_width, fm.horizontalAdvance(DinartetherUnits::name(unit)));
    #else
        max_width = qMax(max_width, fm.width(DinartetherUnits::name(unit)));
    #endif
    }
...
{
    menu = new QMenu(this);
    for (DinartetherUnits::Unit u : DinartetherUnits::availableUnits())
    {
        QAction *menuAction = new QAction(QString(DinartetherUnits::name(u)), this);
...
{
    setText(DinartetherUnits::name(newUnits));
}
...
void DinartetherGUI::currencySelectionChanged(int unitIndex)
{
    if(clientModel && clientModel->getOptionsModel())
    {
...
void DinartetherGUI::onCurrencyChange(int newIndex)
{
    qDebug() << "DinartetherGUI::onPriceUnitChange: " + QString::number(newIndex);
...
void DinartetherGUI::getPriceInfo()
{
...
void DinartetherGUI::getLatestVersion()
{
    versionRequest->setUrl(QUrl("https://api.github.com/repos/DinartetherProject/Dinartether/releases"));
