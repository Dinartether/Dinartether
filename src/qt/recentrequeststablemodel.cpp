    ...
    if(role == Qt::DisplayRole || role == Qt::EditRole)
    {
        ...
        case Amount:
            if (rec->recipient.amount == 0 && role == Qt::DisplayRole)
                ...
                return DinartetherUnits::format(walletModel->getOptionsModel()->getDisplayUnit(), rec->recipient.amount, false, DinartetherUnits::separatorNever);
            else
                return DinartetherUnits::format(walletModel->getOptionsModel()->getDisplayUnit(), rec->recipient.amount);
        }
    }
...
{
    return (this->walletModel->getOptionsModel() != nullptr) ? tr("Requested") + " ("+DinartetherUnits::name(this->walletModel->getOptionsModel()->getDisplayUnit()) + ")" : "";
}
