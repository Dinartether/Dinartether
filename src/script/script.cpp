...
const char* GetOpName(opcodetype opcode)
{
    ...
    /** DINT START */
    case OP_DINARTETHER_ASSET              : return "OP_DINARTETHER_ASSET";
    /** DINT END */
...
{
    if (this->size() > 31) {
        if ((*this)[25] == OP_DINARTETHER_ASSET) { // OP_DINARTETHER_ASSET is always in the 25 index of the script if it exists
            int index = -1;
            if ((*this)[27] == DINARTETHER_R) { // Check to see if DINT starts at 27 ( this->size() < 105)
                if ((*this)[28] == DINARTETHER_V)
                    if ((*this)[29] == DINARTETHER_N)
                        index = 30;
            } else {
                if ((*this)[28] == DINARTETHER_R) // Check to see if DINT starts at 28 ( this->size() >= 105)
                    if ((*this)[29] == DINARTETHER_V)
                        if ((*this)[30] == DINARTETHER_N)
                            index = 31;
            }

            if (index > 0) {
                nStartingIndex = index + 1; // Set the index where the asset data begins. Use to serialize the asset data into asset objects
                if ((*this)[index] == DINARTETHER_T) { // Transfer first anticipating more transfers than other assets operations
                    nType = TX_TRANSFER_ASSET;
                    return true;
                } else if ((*this)[index] == DINARTETHER_Q && this->size() > 39) {
                    nType = TX_NEW_ASSET;
                    fIsOwner = false;
                    return true;
                } else if ((*this)[index] == DINARTETHER_O) {
                    nType = TX_NEW_ASSET;
                    fIsOwner = true;
                    return true;
                } else if ((*this)[index] == DINARTETHER_R) {
...
{
    return (this->size() > 23 &&
            (*this)[0] == OP_DINARTETHER_ASSET &&
            (*this)[1] == 0x14);
}

bool CScript::IsNullGlobalRestrictionAssetTxDataScript() const
{
    // 1 OP_DINARTETHER_ASSET followed by two OP_RESERVED + atleast 4 characters for the restricted name $ABC
    return (this->size() > 6 &&
            (*this)[0] == OP_DINARTETHER_ASSET &&
            (*this)[1] == OP_RESERVED &&
            (*this)[2] == OP_RESERVED);
...
{
    // 1 OP_DINARTETHER_ASSET followed by one OP_RESERVED
    return (this->size() > 3 &&
            (*this)[0] == OP_DINARTETHER_ASSET &&
            (*this)[1] == OP_RESERVED &&
            (*this)[2] != OP_RESERVED);
...
{
    CAmount nAmount;
    return (size() > 0 && *begin() == OP_RETURN) || (size() > 0 && *begin() == OP_DINARTETHER_ASSET) || (size() > MAX_SCRIPT_SIZE) || (GetAssetAmountFromScript(*this, nAmount) && nAmount == 0);
}
