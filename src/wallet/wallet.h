...
#ifndef DINARTETHER_WALLET_WALLET_H
#define DINARTETHER_WALLET_WALLET_H
...
public:
     ...
     * on this dinartether node, and set to 0 for transactions that were created
...
public:
    ...
    void AvailableCoinsAll(std::vector<COutput>& vCoins, std::map<std::string, std::vector<COutput> >& mapAssetCoins,
                            bool fGetDINT = true, bool fOnlyAssets = false,
    ...
    /**
     * Helper function that calls AvailableCoinsAll, used to receive all coins, Assets and DINT
     */
...
#endif // DINARTETHER_WALLET_WALLET_H
