...
BOOST_FIXTURE_TEST_SUITE(asset_tests, BasicTestingSetup)
    ...
    BOOST_AUTO_TEST_CASE(name_validation_tests)
    {
        ...
        //- Versions of DINARTETHER NOT allowed
        BOOST_CHECK(!IsAssetNameValid("DINT", type));
        BOOST_CHECK(!IsAssetNameValid("DINT", type));
        BOOST_CHECK(!IsAssetNameValid("DINARTETHER", type));

        //- Versions of DINARTETHER ALLOWED
        BOOST_CHECK(IsAssetNameValid("DINT.COIN", type));
        BOOST_CHECK(IsAssetNameValid("DINARTETHER_COIN", type));
        BOOST_CHECK(IsAssetNameValid("DINTSPYDER", type));
        BOOST_CHECK(IsAssetNameValid("SPYDERDINT", type));
        BOOST_CHECK(IsAssetNameValid("DINTSPYDER", type));
        BOOST_CHECK(IsAssetNameValid("SPYDEDINT", type));
        BOOST_CHECK(IsAssetNameValid("BLACK_DINTS", type));
        BOOST_CHECK(IsAssetNameValid("SEDINTOT", type));
    ...
    BOOST_AUTO_TEST_CASE(transfer_asset_coin_test)
    {
        ...
        CAssetTransfer asset("DINT", 1000);
    ...
    BOOST_AUTO_TEST_CASE(new_asset_coin_test)
    {
        ...
        // Create the asset scriptPubKey
        CNewAsset asset("DINT", 1000, 8, 1, 0, "");
        CScript scriptPubKey = GetScriptForDestination(DecodeDestination(GetParams().GlobalBurnAddress()));
        asset.ConstructTransaction(scriptPubKey);
